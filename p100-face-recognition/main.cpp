...
tuple<Mat, vector<vector<int>>> getFaceBox(Net net, Mat &frame,
  ...
  while (waitKey(1) < 0) {
    ...
    for (auto it = begin(bboxes); it != end(bboxes); ++it) {
      ...
      */
      ...
      int max_indice_age = std::distance(
      ...
      cv::putText(frameFace, label, Point(it->at(0), it->at(1) - 15),
                  cv::FONT_HERSHEY_SIMPLEX, 0.9, Scalar(0, 255, 255), 2,
                  cv::LINE_AA);
